...
public class CompositionTest {
  static class ServerSpace extends CompositionModule {
    ...
    protected void configure() {
      ...
      bind(ConnectionFactory.class).toComposition(new ConnectionSpace());
    }
  }
  
  static class ConnectionSpace extends CompositionModule {
    @Override
    protected void configure() {
      bind(Connection.class).to(ConnectionImpl.class).in(SINGLETON);
      bind(RequestFactory.class).toComposition(new RequestSpace());
    }
  }
  
  static class RequestSpace extends AbstractModule {
    @Override
    protected void configure() {
  ...
  public void testRuntimeScopes() {
    Server serv = createInjector(new ServerSpace()).getInstance(Server.class);
